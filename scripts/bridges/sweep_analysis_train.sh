#!/bin/bash
# Run spectral analysis for TRAIN rollouts (matches sweep_rollout_train.sh).
#
# Outputs:
#   - ${ANALYSIS_BASE}/${RUN_NAME}-train/
#   - wandb run name: ${RUN_NAME}-train (project: gust2-analysis)
#
# Usage:
#   ./scripts/sweep_analysis_train.sh              Submit all analysis jobs
#   ./scripts/sweep_analysis_train.sh --dry-run
#   ./scripts/sweep_analysis_train.sh --vqvae small-sc341
#   ./scripts/sweep_analysis_train.sh small        Only small NSP models
...
echo "=========================================="
echo "Rollout Spectral Analysis Sweep — TRAIN"
...
for VQVAE_NAME in "${VQVAE_NAMES[@]}"; do
    ...
    for NSP_SIZE in "${NSP_SIZES[@]}"; do
        ...
        ROLLOUT_DIR="${ROLLOUT_BASE}/${RUN_NAME}-train"
        OUTPUT_DIR="${ANALYSIS_BASE}/${RUN_NAME}-train"

        if [ ! -f "${ROLLOUT_DIR}/rollout_tokens.npz" ] && [ "${DRY_RUN}" = false ]; then
            echo "[skip] ${RUN_NAME}-train: no rollout"
            continue
        fi

        if [ -f "${OUTPUT_DIR}/metrics.json" ]; then
            echo "[skip] ${RUN_NAME}-train: analysis exists"
            continue
        fi
        ...
        TMPFILE="$(mktemp /tmp/analysistrain_${RUN_NAME}_XXXXXX.sbatch)"
        cat > "${TMPFILE}" << SBATCH_EOF
#!/bin/bash
#SBATCH -J anatrain-${RUN_NAME}
#SBATCH -A ${ACCOUNT}
#SBATCH -p GPU-shared
#SBATCH -N 1
...
#SBATCH --exclude=w009
#SBATCH -t 2:00:00
#SBATCH -o ${ANALYSIS_BASE}/logs/${RUN_NAME}-train-%j.out
#SBATCH -e ${ANALYSIS_BASE}/logs/${RUN_NAME}-train-%j.err
...
python analyze_rollout.py \\
    ...
    --wandb_group "${VQVAE_SIZE}-nsp-${NSP_SIZE}-train" \\
