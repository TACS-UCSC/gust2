#!/bin/bash
# Run autoregressive rollouts on TRAINING tokens (for overfit check).
# Mirrors sweep_rollout.sh but points at train tokens and writes to
# a -train suffixed output directory.
#
# Usage:
#   ./scripts/sweep_rollout_train.sh              Submit all rollout jobs
#   ./scripts/sweep_rollout_train.sh --dry-run
#   ./scripts/sweep_rollout_train.sh --vqvae small-sc341
#   ./scripts/sweep_rollout_train.sh small        Only small NSP models
...
echo "=========================================="
echo "NSP Rollout Sweep — TRAIN (${N_STEPS} steps, greedy)"
...
for VQVAE_NAME in "${VQVAE_NAMES[@]}"; do
    ...
    for NSP_SIZE in "${NSP_SIZES[@]}"; do
        ...
        OUTPUT_DIR="${ROLLOUT_BASE}/${RUN_NAME}-train"
        ...
        if [ -f "${OUTPUT_DIR}/rollout_tokens.npz" ]; then
            echo "[skip] ${RUN_NAME}-train: rollout already exists"
            continue
        fi
        ...
        TMPFILE="$(mktemp /tmp/rollouttrain_${RUN_NAME}_XXXXXX.sbatch)"
        cat > "${TMPFILE}" << SBATCH_EOF
#!/bin/bash
#SBATCH -J rolltrain-${RUN_NAME}
#SBATCH -A ${ACCOUNT}
#SBATCH -p GPU-shared
#SBATCH -N 1
...
#SBATCH --exclude=w009
#SBATCH -t 12:00:00
#SBATCH -o ${ROLLOUT_BASE}/logs/${RUN_NAME}-train-%j.out
#SBATCH -e ${ROLLOUT_BASE}/logs/${RUN_NAME}-train-%j.err
