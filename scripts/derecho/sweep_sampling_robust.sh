#!/bin/bash
# Same 8-config sampling sweep as sweep_sampling_posmask.sh, but pointed at
# the retrained ar-robust checkpoint (per-position mask in the loss +
# same-position substitution noise on the input). Inference-time
# per-position mask is kept ON so the train/inference vocabularies match.
#
# Designed to be overlaid head-to-head against
# experiments/sampling-sweep-posmask/<RUN>/ (same RUN name, same configs,
# same N_TRAJECTORIES/N_STEPS/SEED) so any improvement is attributable to
# the train-time changes alone.
#
# Submit:
#   qsub scripts/derecho/sweep_sampling_robust.sh
#
# Outputs:
#   $SCRATCH/experiments/sampling-sweep-robust/<RUN>/<cfg_name>/{rollout,analysis}/
#   wandb project: gust2-sampling-robust-derecho
#                  (group=<RUN>, name=<RUN>-<cfg>)
#
#PBS -N sampling-robust
#PBS -A UCSC0009
#PBS -q main
#PBS -l select=1:ncpus=64:ngpus=4:mpiprocs=1:mem=480GB
#PBS -l walltime=02:00:00
#PBS -j oe
#PBS -o /glade/derecho/scratch/anishs/sampling_sweep_robust.out
...
AR_DIR=$SCRATCH/experiments/ar-robust/$RUN
VQVAE_DIR=$SCRATCH/experiments/vqvae/$VQ
WANDB_BASE=$SCRATCH/wandb
WANDB_PROJECT=gust2-sampling-robust-derecho

SWEEP_BASE=$SCRATCH/experiments/sampling-sweep-robust/$RUN
LOG_DIR=$SCRATCH/sampling_sweep_robust_logs/$(date +%Y%m%d-%H%M%S)
mkdir -p $LOG_DIR $SWEEP_BASE

# Sweep configs -- identical to sweep_sampling_posmask.sh so survival
# outcomes are directly comparable cfg-by-cfg.
TASKS=(
  "T10:--temperature 1.0"
  "T09:--temperature 0.9"
  "T08:--temperature 0.8"
...
if [ ! -d "$AR_DIR" ]; then
    echo "ERROR: ar-robust checkpoint dir not found at $AR_DIR"
    echo "       Submit scripts/derecho/train_robust.sh first."
    exit 1
fi
