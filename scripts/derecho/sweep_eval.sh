...
Usage: $0 <sc341|sc917|sc1941> [nano|micro|mini|small|medium|large]
         [--vqvae <size>] [--temperature <T>] [--force] [--dry-run] [--list]

Positional:
  <sc341|sc917|sc1941>          Scale config (required).
  [nsp-size]                    Optional NSP size filter (default: small,
                                medium, large — the historical sizes).
                                Use nano/micro/mini to target the
                                sub-small ablation fleet.
...
# Disable XLA's Triton GEMM backend on A100. eval_single_step.py runs the
# VQ-VAE decoder (transposed convs + codebook one-hot matmul), which
